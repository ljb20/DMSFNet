...
def hard_example_mining(dist_mat, target):
	...
	dist_an1, relative_n_inds = torch.min(
		dist_mat[is_neg].contiguous().view(N, -1), 1, keepdim=True)
...
class QuarCenterTripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        targetsIN = targets[n // 4:n // 2]
        targetsRGB = targets[n // 2:3 * n // 4]
        ...
        dist_pc_dual = (inputsRGBD- center_IR)**2+(inputsIRD- center_RGB)**2
        # dist_pc_dual = (inputsRGB1- centers_dualRGB2)**2+(inputsIR1- centers_dualIR2)**2
    
        # inter——class
        dist_quar = (inputsRGB1 -centers_dualIR1)**2  + (inputsRGB2 - centers_dualIR1)**2  + \
                    (inputsIR1 - centers_dualRGB1)**2  + (inputsIR2 - centers_dualRGB1)**2 
        ...
        loss = tri_loss(inputsIR1,centers_dualRGB1,targetsIN)+tri_loss(inputsRGB1,centers_dualIR1,targetsIN)\
            +tri_loss(inputsIR2,centers_dualRGB1,targetsIN)+tri_loss(inputsRGB2,centers_dualIR1,targetsIN)
