...
class MSEL2(nn.Module):
    def __init__(self,num_pos,feat_norm = 'no'):
        super(MSEL2, self).__init__()
    ...
    def forward(self, inputs, targets):
        ...
        target, target1 = targets.chunk(2,0)
        N = target.size(0)

        dist_mat = pdist_torch(inputs, inputs)

        dist_intra_rgb1 = dist_mat[0 : N, 0 : N]
        dist_cross_rgb1 = dist_mat[0 : N,3*N : 4*N]
        dist_intra_ir1 = dist_mat[N : 2*N, N : 2*N]
        dist_cross_ir1 = dist_mat[N : 2*N, 2*N : 3*N]

        # shape [N, N]
        is_pos = target.expand(N, N).eq(target1.expand(N, N).t())

        dist_intra_rgb1 = is_pos * dist_intra_rgb1
        intra_rgb1, _ = dist_intra_rgb1.topk(self.num_pos - 1, dim=1 ,largest = True, sorted = False) # remove itself
        intra_mean_rgb1= torch.mean(intra_rgb1, dim=1)

        dist_intra_ir1 = is_pos * dist_intra_ir1
        intra_ir1, _ = dist_intra_ir1.topk(self.num_pos - 1, dim=1, largest=True, sorted=False)
        intra_mean_ir1 = torch.mean(intra_ir1, dim=1)

        dist_cross_rgb1 = dist_cross_rgb1[is_pos].contiguous().view(N, -1)  # [N, num_pos]
        cross_mean_rgb1 = torch.mean(dist_cross_rgb1, dim =1)

        dist_cross_ir1 = dist_cross_ir1[is_pos].contiguous().view(N, -1)  # [N, num_pos]
        cross_mean_ir1 = torch.mean(dist_cross_ir1, dim=1)

        loss = (torch.mean(torch.pow(cross_mean_rgb1 - intra_mean_rgb1, 2)) +
                torch.mean(torch.pow(cross_mean_ir1 - intra_mean_ir1, 2))) / 2
