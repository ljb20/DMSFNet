...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.scale = qk_scale or head_dim ** -0.5#通过将其作为缩放因子的倒数平方，可以确保注意力得分在不同头之间具有合适的范围
...
class BlockMask(nn.Module):
    ...
    def forward(self, RGB, NIR, mask, label, epoch=1):
        # RGB_cls = RGB[:, 0, :]
        # NIR_cls = NIR[:, 0, :]
        # loss1 = self.memory_cls(RGB_cls, NIR_cls, label_=label, epoch=epoch)
        ...
        if self.training:
            RGB_cls = RGB[:, 0, :]
            NIR_cls = NIR[:, 0, :]
            loss = self.memory_cls(RGB_cls, NIR_cls, label_=label, epoch=epoch)
            ...
            if x.shape[1] != mask2.shape[1]:
                scale = x.shape[1] // mask2.shape[1]
                mask2 = mask2.repeat(1, scale, 1)
            x = x * mask2
            return x, loss
        else:
            x = torch.cat([RGB, NIR], dim=1)
...
class ViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer = nn.LayerNorm,sie_xishu=1.0,arch='resnet50',cfg=None):
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                v = resize_pos_embed(v, self.pos_embed, self.patch_embed.num_y, self.patch_embed.num_x,self.cls_token_num)
            try:
