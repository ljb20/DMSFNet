...
class Part_Attention(nn.Module):   #局部注意力机制模块的作用是在输入序列中选择具有最大注意力的一部分子序列，并将其作为输出。
...
class SFTS(nn.Module):#根据输入的特征和注意力张量，选择性地提取特定的特征子序列，并将其融合到一起，以产生更具信息量的特征表示。在训练阶段，模块还会通过计算背景损失来监督特征的选择，以确保选择的特征与背景特征之间的一致性。
    def __init__(self, ratio=0.5):
    ...
    def forward(self, RGB_feat, RGB_attn, NIR_feat=None, NIR_attn=None, img_path=None,
